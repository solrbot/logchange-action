...
class GitHubClient:
    ...
    def get_pr_diff(
        self, pr_files: List[str], max_total_tokens: int = 5000, max_per_file: int = 1000
    ) -> str:
        ...
        try:
            ...
            if len(diff_content) > max_total_tokens:
                diff_content = self._truncate_diff(diff_content, pr_files, max_total_tokens, max_per_file)
    ...
    def _truncate_diff(self, diff: str, files: List[str], max_total: int, max_per_file: int) -> str:
...
"""
    ...
    def create_review_comment(self, commit_sha: str, file_path: str, position: int, body: str) -> bool:
