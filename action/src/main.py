...
class LogchangeAction:
    ...
    def __init__(self):
        ...
        self.claude_model = os.getenv('INPUT_CLAUDE_MODEL', 'claude-3-5-sonnet-20241022')
        ...
        # Parse configuration
        self.changelog_types = self._parse_list_input('INPUT_CHANGELOG_TYPES', 'added,changed,deprecated,removed,fixed,security,dependency_update,other')
        self.mandatory_fields = self._parse_list_input('INPUT_MANDATORY_FIELDS', 'title')
        ...
        self.on_legacy_and_logchange = os.getenv('INPUT_ON_LEGACY_AND_LOGCHANGE', 'warn').lower()
        ...
        self.validation_fail_workflow = os.getenv('INPUT_VALIDATION_FAIL_WORKFLOW', 'true').lower() == 'true'

        # Metadata extraction configuration
        self.external_issue_regex = os.getenv('INPUT_EXTERNAL_ISSUE_REGEX', '')
        self.external_issue_url_template = os.getenv('INPUT_EXTERNAL_ISSUE_URL_TEMPLATE', '')
        self.generate_important_notes = os.getenv('INPUT_GENERATE_IMPORTANT_NOTES', 'true').lower() == 'true'
        self.github_issue_detection = os.getenv('INPUT_GITHUB_ISSUE_DETECTION', 'true').lower() == 'true'
        self.issue_tracker_url_detection = os.getenv('INPUT_ISSUE_TRACKER_URL_DETECTION', 'true').lower() == 'true'

        # Initialize clients
        self.github_client = GitHubClient(self.github_token, self.github_api_url, self.github_event)
        ...
        self.metadata_extractor = PRMetadataExtractor(
            external_issue_regex=self.external_issue_regex if self.external_issue_regex else None,
            external_issue_url_template=self.external_issue_url_template if self.external_issue_url_template else None,
    ...
    def run(self) -> int:
        ...
        try:
            ...
            if self._should_skip_changelog(pr_files):
                logger.info('All files match skip pattern, skipping changelog requirement')
                return 0
            ...
            if self.legacy_handler.should_fail_on_conflict(legacy_files, changelog_files):
    ...
    def _is_pr_workflow(self) -> bool:
        ...
        return event_name == 'pull_request' and bool(self.github_event.get('pull_request'))
    ...
    def _report_validation_error(
        ...
        logger.error(f'{error_type.capitalize()} failed for {file_path}: ' + ', '.join(errors))
    ...
    def _handle_missing_changelog(self, pr_files: List[str]) -> int:
        ...
        if self.on_missing_entry == 'fail':
            ...
            try:
                ...
                generated_entry = self.generator.generate(pr_diff, self.github_event.get('pull_request', {}))
                ...
                if not generated_entry:
                    ...
                    self.set_output('generation-error', 'Could not generate valid entry')
                    return 1
...
"""
    ...
    def _handle_legacy_conflict(self, legacy_files: List[str], changelog_files: List[str]) -> int:
        """Handle case where both legacy and logchange entries exist"""
        logger.warning(f'Conflict: {len(legacy_files)} legacy files and {len(changelog_files)} logchange files found')
    ...
    def _handle_legacy_changelog(self, legacy_files: List[str], pr_files: List[str]) -> int:
        ...
        if self.on_legacy_entry == 'warn':
            ...
            if not self.generator:
                logger.error('Legacy conversion requested but no Claude API token provided')
                self.github_client.comment_on_pr(
    ...
    def _convert_legacy_to_logchange(self, legacy_file: str, pr_files: List[str]) -> int:
        ...
        try:
            ...
            if not entry_text:
                logger.error(f'Could not extract changelog entry from diff of {legacy_file}')
                self.github_client.comment_on_pr(
            ...
            generated_entry = self.generator.generate(entry_text, pr_info, custom_prompt=conversion_prompt)
            ...
            suggestion_comment = self._format_legacy_conversion_comment(generated_entry, legacy_file)
            self.github_client.comment_on_pr(suggestion_comment)
    ...
    def _format_legacy_conversion_comment(self, generated_entry: str, legacy_file: str) -> str:
