...
class TestLegacyChangelogHandler:
    ...
    def handler(self):
        ...
        return LegacyChangelogHandler([
            'CHANGELOG.md', 'CHANGELOG.txt', 'CHANGES.md', 'CHANGES.txt',
            'HISTORY.md', 'HISTORY.txt', 'NEWS.md', 'NEWS.txt',
            'RELEASES.md', 'RELEASES.txt'
        ])
    ...
    def test_disabled_handler_returns_empty(self):
        ...
        legacy_files = handler.find_legacy_changelog_files(pr_files)
        assert legacy_files == []

    def test_disabled_handler_with_none(self):
        """Test that handler with None paths is disabled"""
        handler = LegacyChangelogHandler(None)
        assert handler.is_enabled is False
        assert handler.find_legacy_changelog_files(['CHANGELOG.md']) == []

    def test_find_legacy_files_exact_match(self, handler):
        """Test finding legacy files with exact name match"""
        pr_files = ['src/main.py', 'CHANGELOG.md', 'README.md']
        legacy_files = handler.find_legacy_changelog_files(pr_files)
        assert 'CHANGELOG.md' in legacy_files

    def test_find_multiple_legacy_files(self, handler):
        ...
        assert 'CHANGELOG.md' in legacy_files
        assert 'HISTORY.txt' in legacy_files

    def test_find_no_legacy_files(self, handler):
        """Test when no legacy files are present"""
        pr_files = ['src/main.py', 'README.md', 'docs/index.md']
        legacy_files = handler.find_legacy_changelog_files(pr_files)
        assert len(legacy_files) == 0

    def test_find_legacy_files_with_paths(self, handler):
        """Test finding legacy files in subdirectories"""
        pr_files = ['src/main.py', 'docs/CHANGELOG.md', 'docs/README.md']
        custom_handler = LegacyChangelogHandler(['docs/CHANGELOG.md'])
        legacy_files = custom_handler.find_legacy_changelog_files(pr_files)
        assert 'docs/CHANGELOG.md' in legacy_files
...
"""
    ...
    def test_build_legacy_context(self, handler):
        """Test building context about legacy entry"""
        entry = "## Version 1.2.0 - 2024-10-24\n\n### Added\n- Feature 1\n- Feature 2"
        context = handler.build_legacy_context(entry)
    ...
    def test_create_conversion_prompt(self, handler):
        """Test creating conversion prompt"""
        entry = "## Version 1.2.0\n### Added\n- New webhook support"
        pr_info = {'title': 'Add webhook integration'}
        context = handler.build_legacy_context(entry)

        prompt = handler.create_conversion_prompt(entry, pr_info, context)
    ...
    def test_should_fail_on_conflict_true(self, handler):
        """Test conflict detection when both legacy and logchange exist"""
        legacy_files = ['CHANGELOG.md']
        logchange_files = ['changelog/unreleased/change.yml']

        is_conflict = handler.should_fail_on_conflict(legacy_files, logchange_files)
        assert is_conflict is True

    def test_should_fail_on_conflict_false_legacy_only(self, handler):
    ...
    def test_should_fail_on_conflict_false_logchange_only(self, handler):
    ...
    def test_should_fail_on_conflict_false_neither(self, handler):
...
"""
    ...
    def test_entry_context_summary_truncation(self, handler):
        ...
        context = handler.build_legacy_context(long_entry)
