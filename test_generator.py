...
Examples:
  ...
  python3 test_generator.py \\
    ...
    parser.add_argument("--diff", required=True, help="Path to diff file to test")
    parser.add_argument(
        "--pr-info",
        default=None,
    ...
    parser.add_argument(
        "--model", default="claude-opus-4-1-20250805", help="Claude model to use"
    )
    parser.add_argument(
        "--types",
        default=None,
    ...
    parser.add_argument(
        "--mandatory", default=None, help="Comma-separated list of mandatory fields"
    )
    parser.add_argument(
        "--forbidden", default=None, help="Comma-separated list of forbidden fields"
    )
    parser.add_argument(
        "--max-tokens",
        type=int,
    ...
    parser.add_argument(
        "--system-prompt", default=None, help="Custom system prompt file"
    )
    parser.add_argument(
    ...
    if not token:
        ...
        logger.error(
            "  2. Environment variable: export CLAUDE_API_KEY='sk-ant-api03-...'"
        )
        sys.exit(1)
    ...
    logger.info(
        "Using Claude API token from "
        + (
            "command-line argument"
            if args.token
            else "CLAUDE_API_KEY environment variable"
        )
    )
